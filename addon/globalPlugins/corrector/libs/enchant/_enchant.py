...
# On 32-bit NVDA (Python 3.11), apply the bundled ctypes hack if available.
# On 64-bit NVDA (Python 3.13+), the system ctypes works correctly.
_dir_addon = os.path.dirname(__file__)
_bits, _ = platform.architecture()
_311_path = os.path.join(_dir_addon, "..", "_311")
if _bits == "32bit" and os.path.isdir(_311_path):
    sys.path.append(_dir_addon)
    sys.path.append(_311_path)
    import ctypes
    ctypes.__path__.append(os.path.join(_311_path, "ctypes"))
